...
const config = {
  birthdate: 'December 02, 2021',
  name: 'Dương Quang Tùng'
};
...
var x = setInterval(function() {
  ...
  function anim() {
    ...
    if (done) {
      if(tt == 1) opts.strings =["Let's Enjoy",'Your Day!'];
      if(tt == 2) window.location.href = "index2.html";
      update();
      for (let l = 0; l < letters.length; ++l) {
  ...
  if (distance > 0) {
    ...
    function addjustAudioHowl() {
      howl.volume = 0.5;
    }
    function addjustAudioFire() {
      fire.play()
      fire.loop = true;
    }
    init();
